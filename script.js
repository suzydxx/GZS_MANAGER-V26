...
CONTROLE PROFISSIONAL COMPLETO - UPGRADE (VERSÃO BLINDADA + VISUAL FINANCEIRO MELHORADO)
...
function setupLogin(){
  ...
  btn.addEventListener("click", ()=>{
    const userInput = el("user")?.value || "";
    const passInput = el("pass")?.value || "";
...
function renderEmployeeList(){
  ...
  store.employees.forEach(emp=>{
    ...
    const btnAbrir = d.querySelector("button");
...
function renderFuncionario(){
  ...
  function renderTabela(){
    ...
    /* ================================
    RESUMO FINANCEIRO DETALHADO (LAYOUT MELHORADO)
    ================================= */

    let salarioPeriodo = emp.payType === "Semanal" ? 350 : 700;
